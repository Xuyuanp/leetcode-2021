...
class Solution:
    def getIntersectionNode(self, headA: ListNode, headB: ListNode) -> ListNode:
        if not headA or not headB:
            return None
        pa, pb = headA, headB
        # if the tow lists have diff length and intersection node exists,
        # pa and pb will meet in the 2nd loop
        #
        # if there is no intersection node
        # pa's path: listA -> None -> listB -> None
        # pb's path: listB -> None -> ListA -> None
        while pa != pb:
